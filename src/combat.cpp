...
/*
* Get combat damage depending on if you are a monster or a player.
* Depending on if you have min and max values assigned to you via lua.
* minb mina seem to always be 0, could change in future.
* Utilizes formulaType here to figure out which formula to use.
* Returns the raw damage calculation back.
* Check enums for formulaType list
*/
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) { //monsters.cpp is the only thing that sets this value
		...
		int32_t min, max;
		if (creature->getCombatValues(min, max))  //min and max are reference values changed in the call, if player this returns false as its not implemented in player.cpp
		{
			damage.primary.value = normal_random(min, max);
		} 
		else if (Player* player = creature->getPlayer()) //if player, get player object details
		{
			if (params.valueCallback) { //Callback to lua to flesh out minmax values, not needed for src edits?
...
/*
• Sets the formula type to use
• Sets the mina, minb, maxa, maxb values to use
• Interfaces with luascript.cpp and is called there to pull from lua/xml
• Takes the four arguments, which are passed in via somewhere else
*/
...
void Combat::doCombat(Creature* caster, const Position& position) const
{
	...
	if (params.combatType != COMBAT_NONE) { //what type of dmg man
...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		/*
			...
			combatBlockHit => Immunity checks, Manages BlockType Animation
				Calls => blockHit which has 3 implementations, creature.cpp, player.cpp and monster.cpp
					  => this is where dmg is reduced or blocked
		*/
	...
	if (success) {
		...
		if (params.dispelType == CONDITION_PARALYZE) { //why is paralyze hard coded?
			target->removeCondition(CONDITION_PARALYZE);
		} else {
			target->removeCombatCondition(params.dispelType);
		}
