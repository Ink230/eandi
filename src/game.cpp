...
Called from combat.cpp. Handles immunities, ghost modes and if dmg is positive (can't block heals). 
Handles block effect types and animations.
Hands off the nitty gritty of damage reduction via blockHit with 3 implementations in creature.cpp, player.cpp and monster.cpp.

Returns false if either damage was not successfully blocked.
*/
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field, bool ignoreResistances /*= false */)
{
	...
	/* Figuring out the type of block that it was
	 * Rmbr dmg is in negatives
