...
/* TODO
* Find where this is called and update accordingly to account for dual wielding
* The issue is with dual wielding, the right slot weapon won't ever be called
* How do we know to return the correct weapon?
* • Need a player variable to track lasthandused
* • then update lasthandused whenever its time to switch 
*/
Item* Player::getWeapon(bool ignoreAmmo/* = false*/) const
{
	...
	if (isDualWielding())
	{
		return getWeapon(currentAttackHand, ignoreAmmo);
	}
...
/* Modified function
* Added WEAPON_WAND to this check list so getAttackSpeed could use it to 
* scale mlvl wands w/ attack speed 
* Added WEAPON_NONE check for SKILL_FIST
*/
...
/* TODO Believe this already accounts for shields in the left slot
* Thus nothing to change. If shields do not work in the left slot, we need to modify this function.
*/
...
/* Modified function
* Gets vocation AS, weapon skill and calculates a dynamic attack speed value.
* Checks for a weapon type. #WEAPON_TYPE
*
* #Optimize -> smaller variable sizes if possible
* #Lookout -> If

can go negative need to adjust function
*/
uint32_t Player::getAttackSpeed() const
{
	...
	//TODO big long if else check on vocation capped AS limits and modify attspdvalue if below the cap vocation->
...
int32_t Player::getDefense() const
{
	...
	if (weapon) {
		...
		defenseSkill = getWeaponSkill(weapon); //defense then scales off of the weapon's type
	}

	if (shield) {
		defenseValue = weapon != nullptr ? shield->getDefense() + weapon->getExtraDefense() : shield->getDefense(); // IIH -> += here to take into account weapon defense too and not just ignore it? Test having a weapon with 10k def and a shield with 5 def, see which one comes out
		defenseSkill = getSkillLevel(SKILL_SHIELD); //override the weapon skill by prioritizing shield skill if a shield is equipped
	}
...
/*
* Could be issues with this relating to the right defense factor being given.
* It appears that depending on ur attack speed and last time to attack, the defense modifier changes.
* If your AS is faster than ur last attack, ur def is 0.5, if not its 1.0 
*/
			...
			if (slotPosition & SLOTP_RIGHT) {
				if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
					...
					if (leftItem->getSlotPosition() & SLOTP_TWO_HAND) {
						...
						ret = RETURNVALUE_NOERROR;
						// && type != WEAPON_WAND was adjusted here, so wands can be dual wielded 
						// Distance type weapons can never be dual wielded ? 
						//I also took out the distance check, since 2H is a check elsewhere 	
				...
				if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
					...
					if (rightItem->getSlotPosition() & SLOTP_TWO_HAND) {
						...
						ret = RETURNVALUE_CANONLYUSEONESHIELD;
					           ...
					           || type == WEAPON_SHIELD || type == WEAPON_AMMO) {
						ret = RETURNVALUE_NOERROR;
						//TODO same as above comment for right slot 
					} else if (g_config.getBoolean(ConfigManager::ALLOW_DUAL_WIELDING) &&
							 vocation->canDualWield())
					{
						ret = RETURNVALUE_NOERROR;
...
/*
* Attacking
• Checks if a player can attack due to attack speed limits via getAttackSpeed() or other restrictions.
If successful, gets the weapon in use and calls useWeapon which continues the combat tree.
Sets up delays for attack cycles.

Dual Wielding
• Call certain functions twice if dualWielding is true
• Effectively means you attack twice without AS being a factor
• Easier control of 2 weapon handling
*/
...
/*
So you decided to be a big man and attack a creature probably half your size.
Here's all your punishments and restrictions (pz, flags, skull).
*/
...
/*
onAttackedCreature's cousin, Fred.
*/
...
/*
Can you logout? Idk try to avoid all these flags of infight.
*/
...
/*
The simple icon to display you are in a party takes more code than initiating combat.
*/
