...
/*
Loads the weapons array from Lua/xml and populates the corresponding values
...
{
	for (size_t i = 100, size = Item::items.size(); i < size; ++i) { //start i at 100 ?
		const ItemType& it = Item::items.getItemType(i); //get an item property from i
		if (it.id == 0 || weapons.find(i) != weapons.end()) {
			continue; //skip to next item since no item found
		}

		switch (it.weaponType) { //test to see if the retrieved item object has a weapon property
			case WEAPON_AXE:
			case WEAPON_SWORD:
			case WEAPON_CLUB: { //all of these r treated the same
				WeaponMelee* weapon = new WeaponMelee(&scriptInterface); //create weapon object
				weapon->configureWeapon(it); //configure the new weapon object 
				weapons[i] = weapon;
				break;
			}
			...
			default:
				break; //the item in this loop does not have a weapon type property
		}
	}
}
...
/*
Monsters Damage Formula
Monsters.cpp utilizes this function only, since monsters do not use weapons.

• attackSkill is the chosen skill that works with the weapon
• attackValue is the intended weapon attack values
*/
...
/*
Simply sets the id of the weapon to the one found in the item list
Called by all sub-weapon configures
*/
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	if (scripted) {
		...
		if (weaponType == WEAPON_AMMO || weaponType == WEAPON_DISTANCE) { // ORIGIN_SPELL is default value
...
/*
Receives a weapon object and adds properties to it by
the provided weapon/itemobj id

WeaponMelee implementation
*/
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	return -normal_random(0, maxValue); //IIH change this from 0 to something else for more engaging combat?
}
...
/*
Using the weapon for distance.
damageModifier is used to exit out of the method if it is equal to 0.
*/
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	if (item->getWeaponType() == WEAPON_AMMO) { //ammo dmg was add, now find the actual dist weapon by ignoring ammo in getWeapon call
		Item* weapon = player->getWeapon(true); //gets weapon from first or second slot 
		if (weapon) {
			attackValue += weapon->getAttack();
		}
	}
...
bool WeaponWand::getSkillType(const Player *player, const Item *item,
							  skills_t &skill, uint32_t &skillpoint) const
{
	if (player->getAddAttackSkill() && player->getLastAttackBlockType() != BLOCK_IMMUNITY)
	{
	...
	//TODO WEAPON_ROD case? or not needed cause we only use magic level for wands 
	default:
		break;
	}
	return false;
...
{
	int32_t attackSkill = player->getMagicLevel();
	int32_t attackValue = item->getAttack();
	return -attackValue * 100;
	/*
	int32_t attackSkill = player->getMagicLevel();
	if(item->hasAttribute(ITEM_ATTRIBUTE_ATTACK))
	{
		int32_t attackValue = item->getAttack();

		return -(attackValue*attackSkill*10);
		//return -normal_random(minValue, maxValue);
	}
	else
	{
